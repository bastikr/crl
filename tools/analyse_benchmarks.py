...
def parse_arguments() -> Cli:
    parser = argparse.ArgumentParser(
        description="Analyze CMake Resource Library benchmark results."
    )
    parser.add_argument(
        "--benchmark-base-dir",
        type=Path,
    ...
    return Cli(benchmark_base_dir=args.benchmark_base_dir)


def load_data(base_dir: Path) -> list[str]:
    data = []
...
def convert_json_to_data_records(json_data) -> list[DataRecord]:
    records = []
...
def convert_data_records_to_pandas(data_records: list[DataRecord]) -> pandas.DataFrame:
...
def plot_benchmark(df: pandas.DataFrame):
    ...
    worst_benchmarks = df.groupby(
        ["testset_number_of_files", "query_type"], as_index=False
    ).agg({"benchmark_cpu_time_ns": "max"})
...
if __name__ == "__main__":
    ...
    data_records: list[DataRecord] = convert_json_to_data_records(json_data)
    df: pandas.DataFrame = convert_data_records_to_pandas(data_records)
    plot_benchmark(df)
